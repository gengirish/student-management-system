...
import { createStudent, deleteStudent, fetchStudents, uploadProfileImage } from '@/api/client'
...
export function StudentsPage() {
  ...
  const load = useCallback(async () => {
    setError('')
    try {
      const res = await fetchStudents({ page, page_size: 10, q: q || undefined })
  ...
  async function onUpload(studentId: string, file: File | null) {
    ...
    setError('')
    try {
      await uploadProfileImage(studentId, file)
      await load()
    } catch (err) {
          ...
          <Card>
            <CardHeader>
              <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start">
                ...
                <div>
                  <CardTitle>{s.user.full_name}</CardTitle>
            ...
            <CardContent className="space-y-3 text-sm">
              <p>
                <span className="text-zinc-500">Student ID:</span> {s.student_id}
              </p>
              <p>
                <span className="text-zinc-500">Grade / year:</span> {s.grade_year}
              </p>
              <p>
                <span className="text-zinc-500">Enrolled:</span> {s.enrollment_date}
              </p>
              <div className="pt-2">
                ...
                <Input
                  ...
                  disabled={uploading}
                  className="mt-1"
                  onChange={(e) => void onUpload(s.id, e.target.files?.[0] ?? null)}
                />
              </div>
            </CardContent>
          </Card>
      ...
      {user?.role === 'admin' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Creates a login with the student role.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={(e) => void onCreate(e)} className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="cf_pass">Temporary password</Label>
                <Input
                  id="cf_pass"
                  type="password"
              ...
              <div className="space-y-2">
                <Label htmlFor="cf_sid">Student ID</Label>
                <Input
                  id="cf_sid"
              ...
              <div className="space-y-2">
                <Label htmlFor="cf_gy">Grade / year</Label>
                <Input
                  id="cf_gy"
                  type="number"
                  min={1}
                  value={createForm.grade_year}
                  onChange={(e) => setCreateForm((f) => ({ ...f, grade_year: Number(e.target.value) }))}
                  required
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="cf_ed">Enrollment date</Label>
                <Input
                  id="cf_ed"
                  type="date"
                  value={createForm.enrollment_date}
                  onChange={(e) => setCreateForm((f) => ({ ...f, enrollment_date: e.target.value }))}
                  required
                />
              </div>
      ...
      <Card>
        <CardContent className="p-0 pt-0">
            ...
            <TableBody>
              {data?.items.map((s) => (
                <TableRow key={s.id}>
                  ...
                  <TableCell>{s.grade_year}</TableCell>
                  <TableCell>{s.enrollment_date}</TableCell>
                  {user?.role === 'admin' && (
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <label className="cursor-pointer text-sm text-zinc-600 underline dark:text-zinc-400">
                          Photo
                          <input
                            type="file"
                            accept="image/jpeg,image/png,image/webp"
                        ...
                        <Button type="button" variant="destructive" size="sm" onClick={() => void onDelete(s.id)}>
                          Delete
                        </Button>
                      </div>
                    </TableCell>
          ...
          {data && data.items.length === 0 && (
            <p className="p-6 text-center text-sm text-zinc-500">No students found.</p>
          )}
        </CardContent>
      </Card>
